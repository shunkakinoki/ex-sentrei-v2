...
import {remarkToHtml} from "@/utils/remark";
...
export const createMdEmphasis = (num = 3): string => {
  const types = ["_", "~", "*", "**"];
  const word = faker.lorem.word(num);
  ...
  for (let i = 0; num > i; i += 1) {
    ...
    switch (flag) {
      ...
      default: {
        ...
        blocks.push(faker.lorem.paragraphs(Math.floor(Math.random() * 3 + 3)));
      }
    }
  }
  return blocks.join("\n");
...
export const createArticle = async (): Promise<Article> => {
  return {
    body: await remarkToHtml(createMarkdown(30)),
    // body: await createMarkdown(30),
...
export const createArticles = async (num = 6): Promise<Article[]> => {
  return Promise.all(
    new Array(num).fill(undefined).map(async () => {
