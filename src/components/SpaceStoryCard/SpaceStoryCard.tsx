...
export default function SpaceStoryCard({
  ...
  return (
    <Link href={`${namespaceId === "" ? "" : "/"}${namespaceId}/${uid}`}>
      <a
        className={clsx(
          "w-full my-4 px-0 sm:px-2 md:px-3 xl:px-4 shadow-lg sm:shadow-none sm:my-2 ",
          {
            "md:w-1/2 lg:w-1/3": storySwitch && variant === "small",
            "md:w-1/2 lg:w-2/3": storySwitch && variant === "medium",
            "md:w-full py-2": !storySwitch,
          },
        )}
      >
        <div
          className={clsx("rounded sm:rounded-2xl md:rounded-lg group flex", {
            "md:flex-col": storySwitch,
            "md:flex-row items-center": !storySwitch,
          })}
        >
